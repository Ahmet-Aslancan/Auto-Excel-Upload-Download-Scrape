(() => {
  ...
  (function initContentScript() {
    ...
    function inferFieldName(el) {
      return getLabelForElement(el) || normalizeKey(el.getAttribute("aria-label")) || normalizeKey(el.name) || normalizeKey(el.id) || normalizeKey(el.getAttribute("placeholder")) || "";
    }
    function extractPenId() {
      ...
      for (const selector of penSelectors) {
        ...
        if (!el) continue;
        const value = normalizeKey(el.value) || normalizeKey(el.textContent) || normalizeKey(el.getAttribute("data-pen-id"));
      ...
      for (const node of allLabels) {
        ...
        const match = parentText.match(/pen\s*id\s*[:#-]?\s*([A-Za-z0-9_-]+)/i) || parentText.match(/awc\s*child\s*id\s*[:#-]?\s*([A-Za-z0-9_-]+)/i);
        if (match?.[1]) return match[1];
        const sibling = node.nextElementSibling;
        if (sibling) {
          const siblingValue = normalizeKey(sibling.value) || normalizeKey(sibling.textContent) || normalizeKey(sibling.getAttribute("value"));
          if (siblingValue) return siblingValue;
        }
      }
      const bodyText = normalizeKey(document.body?.innerText || "");
      const bodyMatch = bodyText.match(/pen\s*id\s*[:#-]?\s*([A-Za-z0-9_-]+)/i) || bodyText.match(/awc\s*child\s*id\s*[:#-]?\s*([A-Za-z0-9_-]+)/i);
      return bodyMatch?.[1] || "";
    }
    ...
    function extractRowsFromTable(table, headers) {
      ...
      const rawRows = tbodyRows.length > 0 ? tbodyRows : Array.from(table.querySelectorAll("tr")).slice(1);
      const rows = [];
    ...
    function extractStudentTableData() {
      ...
      for (let i = 0; i < tables.length; i += 1) {
        ...
        const headers2 = readHeadersFromTable(table);
        if (!headers2.length) continue;
        const score = scoreStudentTable(headers2);
        if (score > bestScore) {
          bestScore = score;
          best = { table, headers: headers2, index: i };
        }
      }
      if (!best || bestScore < 2) {
    ...
    async function extractStudentTableDataAllPages(options = {}) {
      const limitPages = Number.isInteger(options?.limitPages) && options.limitPages > 0 ? options.limitPages : null;
    ...
    function findLiveRowByPenId(table, penId, penColIndex) {
      ...
      for (const row of rows) {
        ...
        if (!cells.length) continue;
        const penText = penColIndex >= 0 ? normalizePenId(cells[penColIndex]?.textContent || "") : normalizePenId(row.textContent || "");
    ...
    function getTooltipText(el) {
      const svgTitleText = Array.from(el?.querySelectorAll?.("svg title") || []).map((n) => n.textContent).filter(Boolean).join(" ");
      return normalizeKey(
        [
          el?.getAttribute?.("title"),
          el?.getAttribute?.("aria-label"),
    ...
    function findScreeningStartElementInRow(rowEl, screeningColIndex) {
      ...
      const exactSvgTitle = Array.from(candidateScope.querySelectorAll("svg title")).find(
        (n) => /(screening\s*start|start\s*screening)/i.test(normalizeKey(n.textContent)) && !/edit\s*screening/i.test(normalizeKey(n.textContent))
      );
      if (exactSvgTitle) {
        const svg = exactSvgTitle.closest("svg");
        const exactParent = svg && svg.closest("div[title],button,a,[role='button'],div[class*='cursor-pointer']") || getClickableAncestor(svg || exactSvgTitle);
      ...
      const svgTitleNodes = Array.from(candidateScope.querySelectorAll("svg title")).filter(
        (n) => /(screening\s*start|start\s*screening)/i.test(normalizeKey(n.textContent)) && !/edit\s*screening/i.test(normalizeKey(n.textContent))
      );
    ...
    function findLegacyScreeningElementInRow(rowEl, screeningColIndex) {
      ...
      for (const cell of allCells) {
        ...
        const bySvgTitle = Array.from(scope.querySelectorAll("svg title")).find(
          (n) => /(edit|start)\s*screening/i.test(normalizeKey(n.textContent))
        );
        if (bySvgTitle) {
          const clickable2 = getClickableAncestor(bySvgTitle.closest("svg") || bySvgTitle);
          if (clickable2 && isElementVisible(clickable2)) return clickable2;
        }
        const byText2 = Array.from(scope.querySelectorAll("a, button, [role='button'], div, span")).find((el) => {
          if (!isElementVisible(el)) return false;
          const t = (el.textContent || "").replace(/\s+/g, " ").trim();
          return /^(edit\s*screening|start\s*screening)$/i.test(t) || t === "Edit Screening" || t === "Start Screening";
        });
        if (byText2) return getClickableAncestor(byText2) || byText2;
      }
      const directFromLastTd = rowEl.querySelector(
      ...
      const byText = Array.from(
        ...
        return /^(edit\s*screening|start\s*screening)$/i.test(text) || /\b(edit\s*screening|start\s*screening)\b/i.test(text);
      });
      if (byText) return byText;
      const byTextDescendant = Array.from(candidateScope.querySelectorAll("a, button, [role='button'], div, span")).find((el) => {
        if (!isElementVisible(el)) return false;
        const directText = Array.from(el.childNodes).filter((n) => n.nodeType === Node.TEXT_NODE).map((n) => (n.textContent || "").replace(/\s+/g, " ").trim()).join(" ");
    ...
    function hasDetailsFieldMarkers() {
      ...
      return /weight\s*\(?.*kg\)?/i.test(text) || /height\s*\/?\s*length/i.test(text) || /blood\s*pressure/i.test(text) || /vision\s*-\s*left\s*eye/i.test(text);
    }
    ...
    function extractValueFromControlNearLabel(labelRegex, valueParser) {
      ...
      for (const label of labels) {
        ...
        const directControl = label.nextElementSibling?.matches?.("input, textarea, select") ? label.nextElementSibling : null;
        const scopedControl = directControl || label.parentElement?.querySelector?.("input, textarea, select") || label.closest("div")?.querySelector?.("input, textarea, select");
    ...
    function scrapeScreeningDetailsData() {
      ...
      const bmiCalculatedFromPage = extractBmiCalculatedFromControls() || extractValueFromControlNearLabel(/bmi\s*\(?.*calculated.*\)?/i, extractBmiValue) || pickField(allFields, ["bmicalculated", "bmi"], {
      ...
      const bloodPressure = extractBloodPressureFromControls() || extractBloodPressureFromFields(allFields) || extractBloodPressureFromPageText() || pickField(allFields, ["bloodpressure"], {
        excludeAliases: ["classification", "result", "class"]
      });
    ...
    async function waitForStudentTableVisible(timeoutMs = 12e3) {
    ...
    function getCurrentPagePenIds() {
      const ctx = getCurrentTablePageContext();
      if (!ctx) return [];
      return ctx.rows.map((row) => getPenIdFromRowData(row)).map((penId) => normalizePenId(penId)).filter(Boolean);
    }
    ...
    async function goToFirstTablePage() {
      ...
      while (true) {
        ...
        const changed = await waitFor(() => {
          const afterPager2 = getPaginationStateNearNext();
          if (beforePager && afterPager2 && afterPager2.current === 1) return true;
          if (afterPager2 && afterPager2.current < (beforePager?.current ?? 999)) return true;
    ...
    async function submitDetailsThroughPreviewModal() {
      ...
      const confirmVisible = await waitFor(() => Boolean(findConfirmSubmitButton()), 1e4, 120);
      ...
      await waitFor(() => !findConfirmSubmitButton(), 12e3, 120);
      await sleep(500);
    }
    ...
    function getPaginationStateNearNext() {
      ...
      const scope = nextButton.closest("div.flex.flex-col") || nextButton.closest("section") || nextButton.parentElement || document.body;
    ...
    async function goToNextTablePageIfAvailable(_processedPenIdsSet) {
      ...
      const changed = await waitFor(() => {
        const afterPager2 = getPaginationStateNearNext();
        if (beforePager && afterPager2 && afterPager2.current === beforePager.current + 1) return true;
        const currentSignature = getCurrentPageSignature();
        if (Boolean(currentSignature) && currentSignature !== beforeSignature) return true;
        if (beforePager && afterPager2 && afterPager2.current !== beforePager.current) return true;
        return false;
      }, 2e4, 120);
      if (!changed) {
      ...
      const rowsLoaded = await waitFor(() => getCurrentPagePenIds().length > 0, 1e4, 120);
      if (!rowsLoaded) {
    ...
    async function findRowByPenIdAcrossPages(penId) {
      ...
      while (true) {
        const nextStep = await goToNextTablePageIfAvailable(/* @__PURE__ */ new Set());
        if (!nextStep.moved) {
          await goToFirstTablePage();
    ...
    async function extractStudentTableWithDetails(options = {}) {
      ...
      const outRows = [];
      const failures = [];
      const processedPenIds = /* @__PURE__ */ new Set();
      ...
      while (true) {
        ...
        const pendingRows = pageCtx.rows.filter((row) => {
          const penId2 = normalizePenId(getPenIdFromRowData(row));
          return Boolean(penId2) && !processedPenIds.has(penId2);
        });
    ...
    async function waitFor(checkFn, timeoutMs = 4e3, intervalMs = 80) {
    ...
    function setControlValue(control, value) {
      ...
      if (tag === "select") {
        ...
        const direct = options.find(
          (opt) => normalizeHeaderKey(opt.textContent || "") === target || normalizeHeaderKey(opt.value || "") === target
        );
        const partial = direct || options.find(
          (opt) => normalizeHeaderKey(opt.textContent || "").includes(target) || target.includes(normalizeHeaderKey(opt.textContent || "")) || normalizeHeaderKey(opt.value || "").includes(target)
        );
        if (partial) {
      ...
      if (tag === "textarea") {
        const nativeSetter2 = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, "value")?.set;
        if (nativeSetter2) nativeSetter2.call(control, text);
    ...
    async function settleMandatoryScreeningDetails(rowData, timeoutMs = 12e3) {
    ...
    function findHealthConditionsTable() {
      ...
      const healthConditionsH3 = h3s.find(
        (el) => /^health\s*conditions$/i.test(normalizeKey(el.textContent || ""))
      );
      if (healthConditionsH3) {
        const container = healthConditionsH3.parentElement;
        if (container) {
          const table = healthConditionsH3.nextElementSibling?.tagName === "TABLE" ? healthConditionsH3.nextElementSibling : container.querySelector("table");
    ...
    async function waitForHealthConditionsTablePopulated(timeoutMs = 4e3) {
      return waitFor(() => {
    ...
    async function checkHealthConditionByCode(code, defectNameForFallback) {
      ...
      const rawName = String(defectNameForFallback || "").replace(/^\d+\.?\s*/, "").replace(/\s+/g, " ").trim().toLowerCase();
      ...
      for (const tr of rows) {
        ...
        const nameText = tds.length >= 3 ? (tds[2]?.textContent || "").replace(/\s+/g, " ").trim().toLowerCase() : "";
        const matchByCode = targetCode !== "" && codeText === targetCode;
        const matchByName = rawName.length > 0 && (nameText === rawName || nameText.includes(rawName) || rawName.includes(nameText));
        if (matchByCode || matchByName) {
          const matchReason = matchByCode ? "CODE (" + codeText + ")" : 'NAME ("' + nameText + '")';
          tr.scrollIntoView({ block: "center", behavior: "instant" });
          await sleep(100);
          console.warn(
            "[HealthConditions] MATCH by",
            matchReason,
            "| row code:",
            codeText,
            "| row name:",
    ...
    async function handleOtherHealthCondition(defectName) {
      ...
      if (!container) {
        ...
        const label = allLabels.find(
          (l) => /other\s*health\s*condition/i.test(normalizeKey(l.textContent || ""))
        );
    ...
    async function waitForHealthConditionsSection(timeoutMs = 8e3) {
    ...
    async function selectReactSelectOption(container, targetText) {
      ...
      console.warn(
        "[Referral] No matching option found for:",
        targetText,
        "| available:",
    ...
    async function fillReferralDetails(referalValue) {
      ...
      const checkbox = document.getElementById("referSameInstitute") || document.querySelector("input[type='checkbox'][id*='refer']");
      if (checkbox) {
    ...
    async function submitWithMobileNumber(mobileNumber) {
      ...
      const modalReady = await waitFor(() => {
        const confirmBtn2 = findConfirmSubmitButton();
      ...
      if (confirmBtn) {
        ...
        await waitFor(() => !findConfirmSubmitButton(), 12e3, 200);
        await sleep(1e3);
    ...
    async function fillHealthConditionsFromRows(rows) {
      ...
      const birthDefectFoundRaw = rows?.length ? getRowValueByAliases(rows[0], ["birthdefectfound", "birth defect found", "birthdefect"]) : "";
      ...
      if (!section) {
        ...
        const sectionReady = await waitForHealthConditionsSection(15e3);
        if (!sectionReady) {
      ...
      for (const [typeKey, group] of Object.entries(byType)) {
        ...
        await sleep(800);
        const tableReady = await waitForHealthConditionsTablePopulated(12e3);
        if (!tableReady) {
        ...
        for (const row of group.rows) {
          ...
          const directDefectName = defectName || row["Defect Name"] || row["Defect_Name"] || row["DefectName"] || "";
          ...
          console.warn(
            "[HealthConditions] Resolved \u2192 defectName:",
            JSON.stringify(directDefectName),
            "| bestCode:",
            JSON.stringify(bestCode),
            "| bestName:",
            JSON.stringify(bestName)
          );
          const ok = await checkHealthConditionByCode(bestCode, bestName);
          console.warn(
            "[HealthConditions] Result for code:",
            JSON.stringify(bestCode || ""),
            "name:",
            JSON.stringify(bestName),
            "\u2192",
    ...
    async function autoFillRowsOnPage(rows, options = {}) {
      ...
      const tableReady = await waitFor(() => Boolean(getBestStudentTableContext()), 3e3, 100);
      ...
      for (const [penId, groupRows] of penIdGroups) {
        ...
        try {
          ...
          await settleMandatoryScreeningDetails(groupRows[0], 15e3);
          ...
          if (birthNorm) {
            ...
            if (ctrl) {
              await waitFor(() => normalizeHeaderKey(String(ctrl.value || ctrl.textContent || "")) === birthNorm, 6e3, 150);
              await sleep(300);
            }
          }
